...
这个方法被称之为 $\mu Transfer$，基于 Maximal Update Parametrization ($\mu P$) [[1]](#1)。该方法的基本描述如下：
...
|                   $\mu$Transferable                    |            Not $\mu$Transferable            |                   $\mu$Transferred Across                    |
| :----------------------------------------------------: | :-----------------------------------------: | :----------------------------------------------------------: |
...
最左列表示可以直接通过 $\mu P$ 规则进行迁移，中间表示不可迁移，最右列迁移是根据这些参数进行的。
...
而最右列所谓的 $\mu$Transferred Across，是指可迁移的超参数，是在根据一类超参数决定如何迁移的，因为这一类超参数决定了模型的规模。
...
1. 证明了通过 $\mu P$ 进行超参数零样本迁移的可能性。
2. 证明了该方法对于 Adam 同样适用。
3. 提出了 $\mu Transfer$，适用于 SGD 和 Adam。
4. 验证了该方法对于机器翻译、LLM的效果和图像识别一样好。
5. 发布了相关 PyTorch 包。

> 后文可能引起混淆的一些术语：目标模型（Target Model）表示要调优的大模型（Large Model），代理模型（Proxy Model）表示实际代理了调优过程的小模型（Small Model），简化 Transformer 的参数 $d_{model},d_{head}=d_k,d_v,n_{head},d_{ffn}$
...
根据中心极限定理（CLT，Central Limit Theorem），如果我们有 $x_1,\dots,x_n$ 是独立同分布的从一个均值为零、方差为一的分布中采样得到。那么对于 $(x_1+\dots+x_n)$，其均值为零 ，方差为 $n$。这个结果由以下公式得到：
$$
if\quad x_1,x_2,\dots,x_n\quad are \; \text{iid}\; sampled\; from\; \mathcal N(\mu,\sigma^2)\\
S_n=(x_1+\dots+x_n)\\
E(S_n)=E(\sum x_i)=\sum E(x_i)=n\mu\\
Var(S_n)=Var(\sum x_i)=\sum(Var(x_i))=n\sigma^2
$$
如果我们对 $S_n$ 进行缩放，例如对于 $\frac{1}{n}S_n$ 而言，其均值 $E(\frac{1}{n}S_n)=\frac{1}{n}S_n=\frac{\mu}{n}$，而方差 $Var(\frac{1}{n}S_n)=\frac{1}{n^2}Var(S_n)=\frac{\sigma^2}{n}$。

> 注意均值和方差的计算过程

很显然，如果我们不进行缩放，当 $n\to \infin$ 时，那么 $S_n$ 的分布将会发散。或者进行 $\frac{1}{n}$ 缩放，那么 $S_n$ 的方差也会趋向于零，一个均值为零，方差为零的分布是完全没有意义的（或者说）。

当然实际上存在一个缩放，可以保证 $S_n$ 的分布仍然是标准高斯分布，这个缩放因子就是 $\frac{1}{\sqrt(n)}$。

现在我们定义这样一个公式
$$
F_n(c) \overset{def}{=} \mathbb E_{x_1,\ldots,x_n} f\big(c(x_1 + \cdots + x_n)\big)
$$
其中 $f$ 表示一个有界连续函数。我们定义 $c=\frac{\alpha}{\sqrt{n}}$。根据中心极限定理，$G_n(\alpha)=F_n(c)\to \mathbb E f(\mathcal{N}(0, \alpha^2))$ 在 $n\to \infin$ 是一个关于 $\alpha$ 的函数。假设我们要最小化 $G_n(\alpha)$，那么我们会得到 $ \alpha_n^* \overset{def}=arg\min_\alpha G_n(\alpha)$，其中 $\alpha_n^*$ 就是令 $G_n(\alpha)$ 最小化的最优参数。当 n 足够大时，对于任意 $N>n$，甚至 $N\to \infin$，$a_N^*$ 应当非常接近 $a_n^*$。这就意味着，对于一个较小规模的问题，它的缩放参数 $\alpha_n^*$ 和一个较大规模的问题的缩放参数 $\alpha_N^*$ 是近似的。此时我们就可以直接将 $\alpha$ 这个超参数从小模型直接应用到大模型。我们将这样的一个 $c=\frac{\alpha}{\sqrt n}$ 叫做正确的参数化（即我们采用一种方式参数化了 $c$，从而描述了 $c$ 应该如何随着问题规模 $n$ 变化）。

> 这里我们可以将 $x_1,\dots,x_n$ 视为神经网络中的一层神经元。对于小规模网络 n，如果我们为了降低训练损失而对某一层的神经元进行了某种缩放，例如添加了BN 层，此时 $c$ 就是一个超参数，而我们可以通过调节 $c$ 来改变模型的性能。现在我们希望在一个小模型上找到一个最优的缩放因子，使得损失函数最小，然后将其应用到一个更大的模型上，那么通过将这个缩放因子 $c$ 设置为 $c=\frac{\alpha}{\sqrt{n}}$ 的形式，从而使 $c$ 对于规模 n 的变化不敏感。从而实现这个最优 $c$ 可以从小模型直接迁移到大模型的效果。
>
> 为什么分母是 $\sqrt n$？这里因为我们假定是标准高斯分布，通过前面的推导可以发现，分母设置为 $\sqrt n$ 时，缩放之后的分布只和分子相关，而不和问题规模 n 相关。
>
...
在实际使用中，我们通常会加入一个常数因子，来保证上述参数化方法在某些设置下可以同标准参数化方法保持一致。例如引入 $\tilde n \overset{def}=\frac{n}{n_0}$，其中 $n_0$ 表示模型的基本宽度。此时
$$
\text{Initialize:} 
...
$$
在这种定义下，当 $n=n_0$ 时，$\mu Transfer$ 将和标准参数化保持一致，方便进行对比测试。并且在 $n_0$ 上找到的最优超参数，都可以通过上述定义的缩放方法，直接迁移到大模型上使用。

Table.3 给出了通用神经网络下 $\mu P$ 和 SP 的初始化区别。

|  Table.3  | Input weights & all bias |                 Output weights                 |        Hidden weights        |
| :-------: | :----------------------: | :--------------------------------------------: | :--------------------------: |
| Init. Var |   $\frac{1}{fan\_in}$    | $\frac{1}{fan\_in^2}\quad (\frac{1}{fan\_in})$ |     $\frac{1}{fran\_in}$     |
|  SGD LR   |   $fan\_out\quad (1) $   |          $\frac{1}{fan\_in}\quad (1)$          |             $1$              |
|  Adam LR  |           $1$            |          $\frac{1}{fan\_in}\quad (1)$          | $\frac{1}{fan\_in}\quad (1)$ |

其中 $fan\_in$ 表示当前神经元的输入连接数，即上一层的神经元数量。$fan\_out$ 表示当前神经元的输出连接数，即下一层神经元的数量。每一个单元格中，左侧表示 $\mu P$ 的配置，右侧使用括号包括的部分表示对应的 SP 配置。对于 bias 而言其 $fan\_in$ 总是 1。

通常在使用中我们也会引入常数因子来参数化 $fan\_in\; \&\; fan\_out$ 。

Transformer 的配置需要做一点修改：使用 $\frac{1}{d}$ attention 而不是 $\frac{1}{\sqrt d}$。Attention logit 通过 $q^Tk/d$ 计算而不是 $q^Tk\sqrt d$。
...
| :--------: | :----------------------: | :--------------------------: | :--------------------------: |
| Init. Var  |   $\frac{1}{fan\_in}$    | $1\quad (\frac{1}{fan\_in})$ |     $\frac{1}{fran\_in}$     |
| Multiplier |           $1$            | $\frac{1}{fan\_in}\quad (1)$ |             $1$              |
|   SGD LR   |   $fan\_out\quad (1) $   |      $fan\_in\quad (1)$      |             $1$              |
|  Adam LR   |           $1$            |             $1$              | $\frac{1}{fan\_in}\quad (1)$ |

|  Table.9   |          Input weights & all bias          |           Output weights            |        Hidden weights        |
| :--------: | :----------------------------------------: | :---------------------------------: | :--------------------------: |
| Init. Var  | $\frac{1}{fan\_out}\quad\frac{1}{fan\_in}$ |    $1\quad (\frac{1}{fan\_in})$     |     $\frac{1}{fran\_in}$     |
| Multiplier |          $\sqrt {fan\_out}\quad1$          | $\frac{1}{\sqrt{fan\_in}}\quad (1)$ |             $1$              |
|   SGD LR   |            $fan\_out\quad (1) $            |         $fan\_in\quad (1)$          |             $1$              |
|  Adam LR   |    $\frac{1}{\sqrt{fan\_out}}\quad(1)$     | $\frac{1}{\sqrt{fan\_in}}\quad(1)$  | $\frac{1}{fan\_in}\quad (1)$ |
...
$$
这保证了在初始化时 $f(x)=\Theta(\mid X\mid)$，即输出和输入同阶。

我们设置学习率为 1，执行了一步 SGD 之后，新的参数更新为 $V'\gets V+\theta U,U'\gets U+\theta V$。
...
$$
此时注意 $U^TU$ 这一项，由于 $U_{\alpha}\sim \mathcal N(0,1)$，根据大数定理，当 $n\to\infin$ 时，$U^TU=\Theta(n)$，表示这一项是输入 $x$ 的 n 倍高阶，同理加上其他项之后，此时模型的输出将会随着规模的增大发生爆炸。

再考虑使用 $\mu P$ 初始化，即 $V_{\alpha}\sim \mathcal N(0,1/n^2), U\sim\mathcal N(0,1)$，设置学习率 $\eta_V=1/n,\eta_U=n$。此时参数更新为
$$
\begin{aligned}
V' &= V + \eta_V \theta U \\
U' &= U + \eta_U \theta V
